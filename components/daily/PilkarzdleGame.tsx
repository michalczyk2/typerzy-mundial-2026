...
import { FormEvent, useDeferredValue, useState, useSyncExternalStore, useTransition } from 'react'
...
const FIELD_ORDER = ['Narodowosc', 'Pozycja', 'Liga', 'Klub', 'Wiek']
...
const storedGameSnapshotCache = new Map<
  ...
  {
    raw: string | null
    value: PilkarzdleStoredGame | null
  }
>()
...
function readPilkarzdleStats(): PilkarzdleStats {
  ...
  try {
    return JSON.parse(saved) as PilkarzdleStats
  } catch {
    return { playedGames: 0, wonGames: 0, resultsByDay: {} }
  }
}
...
function getComparisonClass(status: PilkarzdleComparisonStatus): string {
  if (status === 'correct') {
    return 'border-emerald-400 bg-emerald-500 text-gray-950 shadow-lg shadow-emerald-950/30'
  }

  if (status === 'close') {
    return 'border-amber-300 bg-amber-400 text-gray-950 shadow-lg shadow-amber-950/25'
  }
...
function filterOptions(
  options: PilkarzdlePlayerOption[],
  query: string,
  ...
  if (!normalizedQuery) {
    return options.filter(option => !guessedIds.has(option.id)).slice(0, 6)
  }

  return options
    .filter(option => !guessedIds.has(option.id))
    ...
    >
      ...
      <div className="relative">
        <p className="text-xs font-black uppercase tracking-[0.24em] text-gray-400">
          {isWon ? 'Brawo!' : 'Koniec prob'}
        </p>
        <h2 className="mt-2 text-2xl font-black text-white">
          {isWon ? 'Odgadles pilkarza dnia.' : 'Poprawna odpowiedz:'}
        </h2>

        <div className="mt-3 flex flex-wrap gap-2">
          <span className="rounded-full bg-gray-900 px-3 py-1 text-xs font-bold text-gray-400">
            {attempts}/{maxAttempts} prob
          </span>
          {isWon && (
...
function FieldCell({ field, fieldIndex }: { field: PilkarzdleGuessField; fieldIndex: number }) {
  const isAge = field.id === 'age'
  const ageArrow = isAge && field.hint ? (field.hint === 'wyzej' ? ' ↑' : ' ↓') : ''
  const hintLabel = isAge && field.hint
    ? (field.hint === 'wyzej' ? 'Starszy' : 'Mlodszy')
    : getStatusLabel(field.status)

  return (
    <div
      className={cn(
        'animate-footwordle-reveal rounded-2xl border p-3 text-center',
        getComparisonClass(field.status)
      )}
    ...
    >
      ...
      <p className="mt-2 min-h-10 text-sm font-black leading-5">
        {field.value}{ageArrow}
      </p>
...
function GuessRow({ guess, index }: { guess: PilkarzdleGuessResult; index: number }) {
  return (
    <article className="animate-footwordle-result overflow-hidden rounded-3xl border border-gray-800 bg-gray-950/60 p-3">
      <div className="grid gap-2 lg:grid-cols-[190px_repeat(5,minmax(100px,1fr))]">
...
export function PilkarzdleGame({ puzzle }: { puzzle: PilkarzdlePublicPuzzle }) {
  ...
  const suggestions = filterOptions(puzzle.candidates, deferredQuery, guessedIds)
  ...
  const visibleMessage =
    message ??
    (status === 'playing'
      ? `Wybierz pilkarza. Zostalo ${puzzle.maxAttempts - guesses.length} prob.`
  ...
  function submitGuess(event?: FormEvent<HTMLFormElement>) {
    ...
    const selectedOption =
      puzzle.candidates.find(option => option.id === selectedPlayerId) ??
      findOptionByInput(puzzle.candidates, query)

    if (!selectedOption) {
      setMessage('Wybierz pilkarza z listy podpowiedzi.')
      return
    }
    ...
    startTransition(async () => {
      try {
        const response = await evaluatePilkarzdleGuess(
          selectedOption.id,
          puzzle.dayKey,
          nextAttemptNumber
        )
        ...
        const revealedAnswer =
          nextStatus === 'playing'
            ? undefined
            : response.result.revealedAnswer ?? response.result.name
        ...
        if (nextStatus !== 'playing') {
          recordPilkarzdleStats(puzzle.dayKey, nextStatus)
        }
        ...
        if (nextStatus === 'won') {
          setMessage(`Brawo! +${points} pkt.`)
        } else if (nextStatus === 'lost') {
          setMessage('Koniec prob. Odpowiedz jest w panelu wyniku.')
        } else {
          setMessage(`Proba ${nextGuesses.length}/${puzzle.maxAttempts}. Szukamy dalej.`)
        }
      } finally {
        setIsChecking(false)
      }
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-start">
      <section className="rounded-3xl border border-gray-800 bg-gray-900 p-4 lg:p-5">
        ...
        <form onSubmit={submitGuess} className="relative rounded-3xl border border-gray-800 bg-gray-950/55 p-4">
          ...
          {query && status === 'playing' && suggestions.length > 0 && (
            <div className="absolute left-4 right-4 top-full z-20 mt-2 overflow-hidden rounded-2xl border border-gray-700 bg-gray-950 shadow-2xl shadow-gray-950/50">
              {suggestions.map(option => (
                <button
                  key={option.id}
                  type="button"
                  onClick={() => selectPlayer(option)}
                  className="block w-full border-b border-gray-800 px-4 py-3 text-left text-sm font-bold text-gray-200 transition last:border-b-0 hover:bg-emerald-950/40 hover:text-emerald-300"
                >
                  {option.name}
                </button>
              ))}
        ...
        <div className="mt-4 hidden grid-cols-[190px_repeat(5,minmax(100px,1fr))] gap-2 px-3 lg:grid">
          ...
          {FIELD_ORDER.map(field => (
            <span
              key={field}
              className="text-center text-[10px] font-black uppercase tracking-[0.2em] text-gray-600"
            >
              {field}
            </span>
          ))}
          ...
          {guesses.length === 0 ? (
            <div className="rounded-3xl border border-dashed border-gray-800 bg-gray-950/35 p-8 text-center">
              <p className="text-sm font-semibold text-gray-400">
                Pierwsza proba odsloni kolory porownan. Zielony = trafione, zolty = blisko,
                czerwony = bledne.
              </p>
            </div>
          ) : (
      ...
      <aside className="space-y-4">
        ...
        <div className="rounded-3xl border border-emerald-900/60 bg-gray-950/80 p-5 shadow-2xl shadow-emerald-950/20">
          ...
          <h2 className="mt-2 text-2xl font-black text-white">8 prób, 5 pól</h2>
          <p className="mt-2 text-sm leading-6 text-gray-400">
            Porownuj narodowosc, pozycje, lige, klub i wiek. Zolty oznacza blisko: ten sam
            kontynent, formacje, kraj ligi, liga albo wiek w zakresie 3 lat.
          </p>
